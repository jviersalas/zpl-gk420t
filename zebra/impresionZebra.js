...
function imprimirZPLPedido(parametros)
{
...
function imprimirRango(doc_num,desde,hasta,co_cli,totalbultos)
{
		checkPrinterStatus( function (text){
		...
		if (text == "Ready to Print" ) {
	        ...
	        $.ajax({
	            data:  parametros, //datos que se envian a traves de ajax
	            url:   '../controlPedido.php?opcion=getZpl', //archivo que recibe la peticion
	            type:  'post', //método de envio
	            beforeSend: function () {
	                $("#resultado").html("Procesando, espere por favor...");
	            },
...
function sendDataUni(co_cli,doc_num,bulto,totalbultos)
{
		checkPrinterStatus( function (text){
		console.log('text: ' + JSON.stringify(text) );
		if (text == "Ready to Print" ) {
...
function sendData(elform)
{
	...
	checkPrinterStatus( function (text){
		...
		if (text == "Ready to Print" ) {
			...
			var co_cli= elform.co_cli.value;
			var doc_num= elform.doc_num.value;
	        ...
	        $.ajax({
	            data:  parametros, //datos que se envian a traves de ajax
	            url:   '../controlPedido.php?opcion=zpl', //archivo que recibe la peticion
	            type:  'post', //método de envio
	            beforeSend: function () {
	                $("#resultado").html("Procesando, espere por favor...");
	            },
	            success:  function (response) { //una vez que el archivo recibe el request lo procesa y lo devuelve
		            ...
		            if (impresos == response.length) {
		            	$("#btn-guardar").fadeOut();
		            	$("#myModal #capa-bultos").fadeOut();
						$("#myModal #capa-siguiente").fadeIn();
						$('#pedidoIr').focus().select();
                           // window.history.back();
                           /* setTimeout(function(){
                                window.history.back();
                           }, 3000);*/
